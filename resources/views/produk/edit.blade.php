...
<div class="row">
    <div class="col-md-12">
        ...
        <form action="/produk/{{$data->id}}" method="post" enctype="multipart/form-data"> 
            ...
            <div class="mb-3">
                <label for="nama_produk">Penjual</label>
                <select class="form-control" name="id_penjual" id="" placeholder="">
                    @foreach ($penjual as $jual)
                        <option   @if ($jual->name == $data->name) selected @endif value="{{ $jual->id }}">{{$jual->name}}  @if ($jual->name == $data->name) (Terpilih) @endif</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
